...
def make_status(root: Path = ROOT) -> dict:
    ...
    return {
        ...
        'risk': {'hard_stop_required': True, 'daily_loss_breaker': True, 'max_leverage': 3, 'max_drawdown': '3%'},
        'validation': [
            {'gate': 'logic_review', 'status': 'implemented', 'reason': 'paper-only safety and code review path exists'},
            {'gate': 'purged_cv', 'status': 'blocked_insufficient_trades', 'reason': 'real-feature evidence has 12 trades on SBTCSUSDT; purged CV requires >=30 and positive expectancy'},
            {'gate': 'deflated_metric', 'status': 'blocked_negative_deflated_sharpe', 'reason': 'orderflow real-feature net -0.165, deflated Sharpe -2.69 (negative)'},
            {'gate': 'walk_forward', 'status': 'blocked_insufficient_out_of_sample', 'reason': '2 folds only; expectancy negative'},
            {'gate': 'shadow', 'status': 'implemented_live_public_readonly', 'reason': 'public multi-exchange + demo-probe read-only paths verified without orders'},
            {'gate': 'micro_live', 'status': 'blocked_ladder_not_passed', 'reason': 'gated controller refuses orders unless promotion evidence has positive expectancy + lower bound + control uplift'},
        ],
        'micro_live_gate': {'credential_present': False, 'demo_product': 'SUSDT-FUTURES', 'promotion_eligible': False, 'note': 'demo-probe is read-only; no order endpoint is ever called without ladder passage'},
