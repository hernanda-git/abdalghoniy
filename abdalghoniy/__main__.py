...
def _replay(args, cfg: AppConfig) -> int:
    ...
    payload = {
        ...
        "fees_slippage_funding": _decimal(costs),
        "net_pnl": _decimal(net),
        "expectancy": _decimal(net / Decimal(len(trades))) if trades else None,
        "trades": [
            {"direction": t.direction, "entry": _decimal(t.entry), "exit": _decimal(t.exit), "quantity": _decimal(t.quantity), "net": _decimal(t.net), "bars_held": t.bars_held}
            for t in trades
        ],
